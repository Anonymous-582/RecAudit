...
from recstudio.ann.sampler import (MaskedUniformSampler, PopularSamplerModel,
                                   UniformSampler)
from recstudio.utils import (DEFAULT_CACHE_DIR, check_valid_dataset, set_color, print_logger,
...
class MFDataset(Dataset):
    ...
    def gen_idx2idx(self, oral_fiid2token2idx=None):

        if oral_fiid2token2idx is not None:
            token2idx = self.field2token2idx[self.fiid]
            keys = list(token2idx.keys())
            idx2token = {}
            for key in keys:
                value = token2idx[key]
                idx2token[value] = key
            self.new_fiid2idx2token = idx2token
            
            assert len(self.new_fiid2idx2token) == len(oral_fiid2token2idx), 'Error'

            self.id2id = torch.LongTensor(list(self.new_fiid2idx2token.keys()))

            for i in range(len(self.new_fiid2idx2token)):
                self.id2id[i] = oral_fiid2token2idx[self.new_fiid2idx2token[i]]

    def trans(self, t):
        return self.id2id[torch.LongTensor(t)]
    ...
    def _load_all_data(self, data_dir, field_sep):
        ...
        if self.frating is None:
            ...
            self.frating = 'rating'
            self.config['rating_field'] = 'rating'
            self.inter_feat.insert(0, self.frating, 0)
    ...
    def __getitem__(self, index):
        ...
        if hasattr(self, 'id2id'):
            if 'user_hist' in data:
                data['user_hist'] = self.trans(data['user_hist'])
            if 'in_' + self.fiid in data:
        ...
        if hasattr(self, 'item_map'):
            data[self.fiid] = self.item_map[data[self.fiid]]

        # if type(self).__name__ == 'SeqDataset':
        #     if data['in_' + self.fiid].shape[1] > self.config['max_seq_len']:
        #         in_item_id = data.pop('in_' + self.fiid)[:, -self.config['max_seq_len']:]
        #         data['in_' + self.fiid] = in_item_id
        #         data['seqlen'] = self.config['max_seq_len'] * torch.ones_like(data['seqlen']).to(data['in_' + self.fiid].device)

        if hasattr(self, 'user_no_hist') and self.user_no_hist == True:
            #data['user_hist'] = torch.zeros_like(data['user_hist'])
    ...
    def build(self, split_ratio=[0.8,0.1,0.1], shuffle=True, split_mode='user_entry', fmeval=False, dataset_sampler=None, dataset_neg_count=None, **kwargs):
        """Build dataset.
    ...
    def loader(self, batch_size, shuffle=True, num_workers=1, drop_last=False, ddp=False, active=None):
        if hasattr(self, 'user_batch_size') and hasattr(self, 'active_uid2sample') and hasattr(self, 'active_users'):
            sampler = UserSampler(self, batch_size, shuffle, drop_last)
        elif hasattr(self, 'active_index'):
            sampler = SortedDataSampler(self, batch_size, shuffle, drop_last, index=self.active_index)
        else:
    ...
    def sample_length(self):
        if self.data_index.dim() > 1:
            ...
            if type(self).__name__=='MFDataset':
                return torch.randint(10, size=(len(self.data_index),))
            else:
    ...
    def eval_loader(self, batch_size, num_workers=1, ddp=False):
        if not getattr(self, 'fmeval', False):
            ...
            if hasattr(self, 'active_index'):
                sampler = SortedDataSampler(self, batch_size, index=self.active_index)
            else:
                sampler = SortedDataSampler(self, batch_size)
            if ddp:
                sampler = DistributedSamplerWrapper(sampler, shuffle=False)
            output = DataLoader(
                self, sampler=sampler, batch_size=None, shuffle=False, num_workers=num_workers)
            return output
        else:
...
class UserSampler(Sampler):
    ...
    def __init__(self, data_source: Sized, batch_size, shuffle=True, drop_last=False, generator=None):
        ...
        self.active_users = data_source.active_users
        self.active_uid2sample = data_source.active_uid2sample
        self.batch_size = data_source.user_batch_size

    def __user2sample__(self, outputs):
        sample_outputs = []
        for user_indexes in outputs:
            sample_output = []
            user_indexes = user_indexes.numpy()
            for index in user_indexes:
                user = self.active_users[index]
                sample_output += self.active_uid2sample[user]
            sample_outputs.append(torch.tensor(np.array(sample_output)))
        return sample_outputs

    def __iter__(self):
        n = len(self.active_users)
        if self.generator is None:
            generator = torch.Generator()
            generator.manual_seed(
                int(torch.empty((), dtype=torch.int64).random_().item()))
        ...
        output = self.__user2sample__(output)
...
class SortedDataSampler(Sampler):
    ...
    def __init__(self, data_source: Sized, batch_size, shuffle=False, drop_last=False, generator=None, index=None):
        self.data_source = data_source
        self.batch_size = batch_size
        self.drop_last = drop_last
        self.shuffle = shuffle
    ...
    def __iter__(self):
        ...
        if self.index is not None:
            _, ind, _ = np.intersect1d(output.numpy(), self.index, assume_unique=True, return_indices=True)
            output = output[np.sort(ind)]

...
def agreement(pred, target, k):
    r"""Calculating agreement measure

    """
    device = pred.device
    pred_np = pred[:,:k].detach().clone().cpu().numpy()
    target_np = target[:,:k].detach().clone().cpu().numpy()

    batch_size, _ = pred_np.shape
    agg_score = []
    for i in range(batch_size):
        agg_score.append(len(set(pred_np[i]).intersection(set(target_np[i]))) / k)
    agg_score = np.mean(np.array(agg_score))
    return torch.FloatTensor([agg_score]).to(device)[0]
...
def ndcg(pred, target, k):
    ...
    ideal_dcg = _dcg(torch.sort((target > 0).float(), descending=True)[
                     0], k)  # to do replace target>0 with target
...
def get_rank_metrics(metric):
    if not isinstance(metric, list):
        metric = [metric]
    topk_metrics = {'ndcg', 'precision', 'recall', 'map', 'mrr', 'hit','agg'}
